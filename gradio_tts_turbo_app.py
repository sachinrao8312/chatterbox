...
print("="*50)
print(f"Diagnostics: Python {sys.version}")
...
# NOTE: We avoid torch.set_default_device("cuda") as it can conflict with 
# libraries like Gradio/Numpy that expect CPU tensors by default.
# The TTS model handles .to(device) explicitly.
...
def generate(
    ...
    print(f"\n{'='*60}")
    print(f"🎯 CUDA CORES USAGE CHECK (Turbo TTS)")
    print(f"{'='*60}")
    ...
    if torch.cuda.is_available():
        ...
        print(f"\n{'='*60}")
        print(f"🚀 CUDA GENERATION COMPLETE (Turbo TTS)")
        print(f"{'='*60}")
    ...
    audio_saver.save_audio_output(model.sr, audio_data, "Turbo")
